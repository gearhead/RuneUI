...
// May need to check to see if an iwd config is here but not autoconnect
            netd_config($redis, $args);
            if (strpos($args['nic'], 'wlan') === 0) {
                connectWifi($redis, $args);
            }
            break;
        case 'autoconnect-on':
...
//            }
        ...
        case 'delete':
            ...
            if (isset($storedProfiles[$ssidHexKey])) {
                ...
                // for networkd, we create 20-$nic.network files if one exists, remove it
...
//                sysCmd('connmanctl config '.$args['connmanString'].' --remove');
                ...
                $network_info = json_decode($redis->get('network_info'), true);
                foreach ($network_info as $network) {
                    if ($network['ssidHex'] != $storedProfiles[$ssidHexKey]) {
...
//                    wrk_systemd_unit($redis, 'start', 'connman');
            ...
            // then it gets a 20-$args['nic'],network value and uses the specific mac to the interface
...
function wrk_shairport($redis, $ao = null, $name = null, $jobID = null)
{
    ...
    if (!isset($acard) || !is_array($acard) || !isset($acard['sysname'])) {
        ...
        if ($redis->hGet('owntone', 'active')) {
            ...
            $acard['device'] = $redis->hGet('owntone', 'device_ap');
            $acard['extlabel'] = 'Owntone';
            $acard['sysname'] = 'owntone';
            $acard['type'] = 'alsa';
            $acard['description'] = 'Owntone';
    ...
    switch ($action) {
        ...
        case 'status':
            ...
            if ($server && wrk_systemd_unit($redis, 'is-active', 'owntone') && ($role == 'server')) {
                ...
                $retval = sysCmd('curl -X GET -s "http://'.$server.':3689/api/config"');
                if (!$retval || !is_array($retval)) {
                    $retval = sysCmd('curl -X GET -s "http://'.$server.':3689/api/config"');
                }
                if ($retval && is_array($retval)) {
