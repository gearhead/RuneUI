...
{
    if (substr($path, 0, 5) == '/var/') {
        // the specified socket is a socket file, not a web address
        clearstatcache(true, $path);
        if (!file_exists($path)) {
            // the socket file does not exist
            runelog("[open]\t>>>>>> OPEN MPD SOCKET ERROR - **Socket path invalid, no file found = ".$path."** <<<<<<",'');
            return false;
        }
    }
...
function sendMpdIdle($sock)
{
    ...
    if (strpos(' '.$resp, 'OK')) {
        ...
        // convert it to a string delimited by colons, no duplicates, e.g. 'player:playlist:mixer:OK', not 'player:playlist:player:mixer:OK'
        $plistArray = array();
...
{
    // debug
    // echo 'sysCmdAsync, '.date("Y-m-d G:i:s").': "'.$syscmd."\"\n";
...
function wrk_i2smodule($redis, $args = null, $jobID = null)
{
    ...
    if($redis->get('hwplatformid') === '01' || $redis->get('hwplatformid') === '08') {
        ...
        if (strpos(' '.strtolower($args), 'hifiberry')) {
            // some hifiberry dacs have an eeprom which identifies the dac, this must disabled for the Pi5
            //  the changed lines are contained in conditional statements for specific models
...
//  forcerestart
{
    switch ($action) {
        ...
        case 'writecfg':
            ...
            foreach ($acards as $main_acard_name => $main_acard_details) {
                ...
                // } else {
                    // $output .="\tenabled \t\"no\"\n";
                // }
                // allways set up all cards as disabled, except null and httpd streaming
                //  this prevents mpd restarts triggered by changes output selection changes
                //  the active card is enabled after restarting mpd
                $output .="\tenabled \t\"no\"\n";
                $output .="}\n";
                unset($sub_interface);
                // debug
        ...
        case 'switchao':
            ...
            if (!isset($args) || !$args || !$redis->hExists('acards', $args)) {
                if ($oldMpdout && $redis->hExists('acards', $oldMpdout)) {
                    $args = $oldMpdout;
                } else {
                    $ao_default = trim($redis->get('ao_default'));
                    if ($ao_default && $redis->hExists('acards', $ao_default)) {
                        $args = $ao_default;
                    } else {
                        $args = '';
                    }
            ...
            if ($args) {
                ...
                // invalid device (could happen, e.g. owntone server with no local outputs)
...
function wrk_spotifyd($redis, $ao = null, $name = null, $jobID = null)
{
    ...
    // record current interface selection, note: $ao can be an empty string!
    $oldMpdout = trim($redis->get('ao'));
    if (!isset($ao) || !$ao || !$redis->hExists('acards', $ao)) {
        if ($oldMpdout && $redis->hExists('acards', $oldMpdout)) {
            $ao = $oldMpdout;
        } else {
            $ao_default = trim($redis->get('ao_default'));
    ...
    if (isset($acard['swdevice'])) {
        ...
        // no output card available stop spotifyd and exit
        wrk_systemd_unit($redis, 'stop', 'spotifyd');
        return 0;
    }
    //
    if (isset($acard['mixer_control']) && !empty($acard['mixer_control'])) {
    ...
    $spotifyd_conf .= '# See: https://github.com/Spotifyd/spotifyd/blob/master/contrib/spotifyd.conf'."\n";
...
function wrk_shairport($redis, $ao = null, $name = null, $jobID = null)
{
    // debug
    ...
    foreach ($deduplicatedNetworks as $wifiNet) {
        ...
        if ($security !== '' && $security !== '--') {
            if (strpos($security, 'WPA3') !== false) {
                ...
                $securityType = 'WEP';
            } elseif (strpos($security, '802.1X') !== false) {
        ...
        default:
            ...
            if (($redis->hGet('globalrandom', 'enable')) && (!$redis->hGet('globalrandom', 'wait_for_play'))) {
                ...
                if (wrk_systemd_unit($redis, 'is-active', 'ashuffle')) {
                    ...
                    } else if (!sysCmd('mpc outputs | grep -ic enabled | xargs')[0]) {
...
// this function also manages cache files containing all the retrieved information for a given webradio string
{
    ...
    $artDir = rtrim(trim($redis->get('albumart_image_dir')), '/');
    $fileName = $artDir.'/none.png';
    clearstatcache(true, $fileName);
    if (!file_exists($fileName)) {
        // metadata cache has not yet been initialised (on startup)
        // so just return false
        return 0;
    }
    $info = array();
       ...
       case 'connect':
        ...
        case 'status':
            ...
            if (!$mpdConfigured) {
                ...
                sysCmdAsync($redis, '/srv/http/command/refresh_ao', 0);
            }
            wrk_btcfg($redis, 'correct_bt_ao');
            $redis->set('bluetooth_status', json_encode($deviceArray));
            $retval = $deviceArray;
        ...
        case 'status_async':
            ...
            sysCmdAsync($redis, '/srv/http/command/bt_status_async.php');
            break;
        case 'status_async_now':
            // runs the status option asynchronously now
            sysCmdAsync($redis, '/srv/http/command/bt_status_async.php', 0);
            break;
...
// 'hifiberry-dacplus,slave|Argon BLSTR DAC for Pi 5' valid only for Pi5
// 'hifiberry-dacplus|Argon BLSTR DAC' invalid for Pi5
...
//
{
    ...
    if (substr($i2smodule_select, 0, 5) != 'none|') {
        ...
        if ($i2smodule_select == 'allo-katana-dac-audio|Inno-Maker Raspberry Pi HiFi DAC Pro HAT ES9038Q2M') {
            ...
            $reboot = true;
        } else if ($i2smodule_select == 'hifiberry-dacplus|Argon BLSTR DAC') {
            wrk_i2smodule($redis, 'hifiberry-dacplus,slave');
            $redis->set('i2smodule_select', 'hifiberry-dacplus,slave|Argon BLSTR DAC for Pi 5');
            $reboot = true;
        } else if ($i2smodule_select == 'pisound|Blokas Labs pisound card') {
...
// 'hifiberry-dacplus,slave|52Pi NVDAC for Pi 5' valid only for Pi5
// 'hifiberry-dacplus,slave|Argon BLSTR DAC for Pi 5' valid only for Pi5
// 'hifiberry-dacplus,slave|Argon BLSTR DAC' invalid for Pi5
// 'hifiberry-dacplus|52Pi NVDAC' invalid for Pi5
// 'hifiberry-dac|Pine HatDrive! Piano' valid only for Pi5
// 'inno-dac-pro|Inno-Maker Raspberry Pi HiFi DAC Pro HAT ES9038Q2M' valid only for Pi5
// 'pisound-pi5|Blokas Labs pisound card' valid only for Pi5
...
{    if ($redis->get('aocardexceptions') == 'Non-Pi5') {
    ...
    sysCmd("sed -i '/hifiberry-dacplus,slave|Argon BLSTR DAC for Pi 5/d' '/srv/http/.config/i2s_table.txt'");
    ...
    if (substr($i2smodule_select, 0, 5) != 'none|') {
        // i2s card is set, maybe we need to change config.txt and the i2smodule and i2smodule_select variables
        if ($i2smodule_select == 'hifiberry-dac8x|HiFiBerry DAC8X') {
            wrk_i2smodule($redis, 'none');
            $redis->set('i2smodule_select', 'none|I\xc2\xb2S disabled (default)');
            $reboot = true;
        } else if ($i2smodule_select == 'hifiberry-dac|Pine HatDrive! Piano') {
            wrk_i2smodule($redis, 'none');
            $redis->set('i2smodule_select', 'none|I\xc2\xb2S disabled (default)');
            $reboot = true;
            ...
            $reboot = true;
        } else if ($i2smodule_select == 'hifiberry-dacplus,slave|Argon BLSTR DAC for Pi 5') {
            wrk_i2smodule($redis, 'hifiberry-dacplus');
            $redis->set('i2smodule_select', 'hifiberry-dacplus|Argon BLSTR DAC');
            $reboot = true;
            ...
            $reboot = true;
        } else if ($i2smodule_select == 'pisound-pi5|Blokas Labs pisound card') {
    ...
    switch ($action) {
        case 'activate':
            ...
            if ($redis->hGet('owntone', 'enable')) {
                ...
                // save the current airplay pipe output and set the airplay pipe output to that of owntone
                $airplaySavedPipeName = $redis->hGet('owntone', 'saved_airplay_pipe_name');
                $airplayPipeName = $redis->hGet('airplay', 'pipe_pipe_name');
                $owntonePipeName = $redis->hGet('owntone', 'pipe_ap');
                if ($owntonePipeName != $airplayPipeName) {
                    $redis->hSet('airplay', 'pipe_pipe_name', $owntonePipeName);
                    if (!$airplaySavedPipeName) {
                        $redis->hSet('owntone', 'saved_airplay_pipe_name', $airplayPipeName);
                    }
                }
                // save the current spotify connect volume control and set the current spotify connect volume control to 'none'
                $spotifyconnectSavedVolumeControl = $redis->hGet('owntone', 'saved_spotifyconnect_volume_control');
                $spotifyconnectVolumeControl = $redis->hGet('spotifyconnect', 'volume_control');
                if ($spotifyconnectVolumeControl != $spotifyconnectVolumeControl) {
                    $redis->hSet('spotifyconnect', 'volume_control', 'none');
                    if (!$spotifyconnectSavedVolumeControl) {
        ...
        case 'deactivate':
            ...
            // remove any mute entries and correct preset volume in the presets
            ...
            // set the airplay pipe name to its original value
            $airplaySavedPipeName = $redis->hGet('owntone', 'saved_airplay_pipe_name');
            $airplayPipeName = $redis->hGet('airplay', 'pipe_pipe_name');
            if ($airplaySavedPipeName && ($airplaySavedPipeName != $airplayFormat)) {
                $redis->hSet('airplay', 'pipe_pipe_name', $airplaySavedPipeName);
            }
            // set the spotify connect volume control to its original value
            $spotifyconnectSavedVolumeControl = $redis->hGet('owntone', 'saved_spotifyconnect_volume_control');
            $spotifyconnectVolumeControl = $redis->hGet('spotifyconnect', 'volume_control');
            if ($spotifyconnectSavedVolumeControl && ($spotifyconnectSavedVolumeControl != $spotifyconnectVolumeControl)) {
                $redis->hSet('spotifyconnect', 'volume_control', $spotifyconnectSavedVolumeControl);
            }
        ...
        case 'disable':
            ...
            // stop owntone
            wrk_systemd_unit($redis, 'stop', 'owntone owntone_monitor');
            // owntone is disabled the symlink shairport-sync.service must point to shairport-sync-ap2.service when the shairport-sync configuration is dual
            if ($redis->hGet('airplay', 'ss_conf') == 'dual') {
                // its a dual shairport-sync configuration. it is possible that the symlink is correct, this happens when the override option is set
                $filename = '/etc/systemd/system/shairport-sync.service';
                clearstatcache(true, $filename);
                $symlinkValue = readlink($filename);
                $filename1 = '/etc/systemd/system/shairport-sync-ap2.service';
                clearstatcache(true, $filename1);
                if ($filename1 != $symlinkValue) {
                    // the symlink is pointing to the wrong file
                    if ($redis->get('activePlayer') == 'Airplay') {
                        // Airplay is active, stop the stream and switch the playback engine to MPD
                        sysCmd('/srv/http/command/airplay_toggle off');
                    }
                    wrk_systemd_unit($redis, 'stop', 'shairport-sync nqptp');
                    // remove the systemd unit file for shairport-sync, normally a symlink
                    if (file_exists($filename)) {
                        unlink($filename);
                    }
                    // check that the shairport-sync-ap2.service file exists
        ...
        case 'status':
            ...
            if ($redis->hGet('owntone', 'active')) {
                ...
                $serverHostname = $redis->get('hostname');
                $redis->hSet('owntone', 'server_hostname', $serverHostname);
                $network_interfaces = json_decode($redis->get('network_interfaces'), true);
                unset($ipAddress);
                foreach ($network_interfaces as $network_interface) {
                    if (!isset($ipAddress)) {
                        if (isset($network_interface['ipv4Address'])) {
                            $ipAddress = $network_interface['ipv4Address'];
                        }
                    } else if (isset($network_interface['technology']) && ($network_interface['technology'] == 'ethernet')) {
                        if (isset($network_interface['ipv4Address'])) {
                            $ipAddress = $network_interface['ipv4Address'];
                        }
                    }
            ...
            if (isset($server) && $server && wrk_systemd_unit($redis, 'is-active', 'owntone') && ($role == 'server')) {
                ...
                if (isset($serverPlayer['volume'])) {
                    ...
                    if (is_firstTime($redis, 'MR_owntone_master_volume')) {
                        // save the server information (and master volume)
                        $master['volume'] = $serverPlayer['volume'];
                        if ($master['volume']) {
                            $master['mute'] = 0;
                        }
                    ...
                    foreach ($retval['outputs'] as $output) {
                        ...
                        if ($output['name'] == $redis->hGet('airplay', 'name')) {
                            ...
                            if ($redis->hExists('owntone_presets', $output['name'])) {
                                $redis->hDel('owntone_presets', $output['name']);
                            }
                            continue;
                        }
                        $setvolume = false;
                        ...
                        if (isset($output['name']) && $output['name']) {
                            ...
                            $detectedOutputNames[] = $output['name'];
                        ...
                        $presetInitial = $preset;
                        if ($output['type'] == 'ALSA') {
                            // ALSA outputs
                            //  when $multidevice is set multiple local outputs are valid
                            //  when $multidevice is not set only one local outputs should be enabled, disable others
                            if ($multidevice) {
                                ...
                                if (!$output['selected'] && ($output['name'] == $localOutputName) && is_firstTime($redis, 'MR_alsa_autoconnect_'.$output['id'])) {
                                    ...
                                    if ($preset['autoconnect'] && isset($preset['volume_preset']) && is_numeric($preset['volume_preset']) && !$useCurrentVolume) {
                                        ...
                                        // use the default volume when no others are valid
                                    ...
                                    if (isset($preset['volume_preset']) && is_numeric($preset['volume_preset'])) {
                                        // autoconnect, use the preset volume when valid
                                        $volume = $preset['volume_preset'];
                                    } else {
                                        // use the default volume when the preset volume is invalid
                                        $volume = $defaultVolume;
                                        if (!$preset['autoconnect']) {
                                            // not autoconnect so set the preset volume to the default volume
                                            $preset['volume_preset'] = $defaultVolume;
                                ...
                                $preset['volume_preset'] = $defaultVolume;
                                if ($output['selected'] && ($output['name'] != $localOutputName)) {
                                    // this one is connected should not be be
                                    $disconnect = true;
                                    $setvolume = true;
                                    $volume = $defaultVolume;
                                    $preset['mute'] = 0;
                                } else if (!$output['selected'] && ($output['name'] == $localOutputName)) {
                            ...
                            if (!$output['selected'] && $preset['autoconnect']
                                ...
                                if (isset($nodeInfo) && isset($nodeValue) && ($nodeInfo == 'volume') && is_numeric($nodeValue)) {
                                    ...
                                    // use the default volume when the preset volume is invalid
                                    $volume = $defaultVolume;
                                    // set the preset volume to default volume
                                    $preset['volume_preset'] = $defaultVolume;
                                }
                                if ($volume) {
                                    // remove mute when volume is non zero
                        ...
                        // only connect the first autoconnected bluetooth device
                        if (substr($output['name'], 0, 11) == 'Bluetooth: ') {
                            // it is a bluetooth output device
                            if ($bluetoothConnected) {
                                // there is already a bluetooth output device connected
                                if ($autoconnect) {
                                    // this is a subsequent bluetooth output device with autoconnect
                                    $autoconnect = false;
                                } else if ($output['selected']) {
                                    // this is a subsequent bluetooth output device which is connected
                                    $disconnect = true;
                                }
                            } else {
                                // no bluetooth devices have been detected as connected
                                if ($autoconnect || $output['selected']) {
                                    // this one will be connected or is already connected
                                    $bluetoothConnected = true;
                                }
                            }
                        }
                        if ($presetInitial != $preset) {
                            // save the preset information when required
                            $redis->hSet('owntone_presets', $output['name'], json_encode($preset));
                        }
                        ...
                        // correct the mute if required
                        if (!isset($output['volume']) || ($output['volume'] != 0)) {
                            // no output or output volume non-zero, correct mute
                            if ($preset['mute']) {
                                $preset['mute'] = 0;
                ...
                if (is_firstTime($redis, 'MR_owntone_unmute')) {
                    foreach ($presetNames as $presetName) {
                        if (in_array($presetName, $detectedOutputNames)) {
                            // skip presets for detected outputs, this check has already been done
                            continue;
                        }
                        $preset = json_decode($redis->hGet('owntone_presets', $presetName), true);
                ...
                // remove any invalid outputs from redis
                $outputNames = $redis->hKeys('owntone_outputs');
                if (is_array($outputNames) && count($outputNames)) {
                    foreach ($outputNames as $outputName) {
                        if (in_array($outputName, $detectedOutputNames)) {
                            // this one is valid
                            continue;
                        }
                        if ($redis->hExists('owntone_outputs', $outputName)) {
                            $redis->hDel('owntone_outputs', $outputName);
                        }
                    }
                }
                // adjust the master volume and mute values
                if ($numberOutputs) {
                    // calculate the average volume
                    //  $totalOutputVolume is the sum of all active 'output volume' plus all active 'mute value'
                    //      per output only one of 'output volume' or 'mute value' will be set the other will be zero
                    // $averageVolume is the volume if all active outputs were unmuted
                    $averageVolume = round($totalOutputVolume / $numberOutputs);
                    // just to be safe $averageVolume has a range of 0 to 100
                    ...
                    if ($unmuteMaster) {
                        ...
                        // all outputs are muted treat master as muted
                        $master['volume'] = 0;
                        $master['mute'] = $averageVolume;
                    }
                    $redis->hSet('owntone', 'master', json_encode($master));
                }
            } else {
        ...
        case 'muteasync':
            ...
            //  mute with 'unmute' is used when pausing or stopping the player of switching outputs, owntone will
            //      normally continue playing until the buffer empties
            //      muting an unmuting gives the impression of responsiveness to UI commands
            //  muting is carried out per output device not using the master mute
            // check that owntone is running and is active
            // this async version of the function starts the command file 'owntone_mute.php' asynchronously which in turn
            //  calls wrk_owntone($redis, 'mute', $args);
            // check that owntone is running and is active
            if (!$redis->hGet('owntone', 'active') || !wrk_systemd_unit($redis, 'is-active', 'owntone')) {
                // owntone is not running, do nothing
                break;
            }
            if (isset($jobID) && $jobID) {
                $redis->sRem('w_lock', $jobID);
            ...
            // run the command file asynchronously without queueing and with 0 seconds delay
        ...
        case 'mute':
            ...
            foreach ($outputNames as $outputName) {
                ...
                if (!$output['selected'] || ($output['volume'] == 0)) {
            ...
            if (isset($args) && ($args == 'unmute') && $mutedIds) {
                // unmute set and there is something to unmute
                $mutedIds = 'delay'.$mutedIds;
                wrk_owntone($redis, 'unmuteasync', $mutedIds);
            }
            $master = json_decode($redis->hGet('owntone', 'master'), true);
        ...
        case 'unmuteasync':
            ...
            //  $args can be prefixed with 'delay(-)' which results in the asynchronous unmute action being delayed
            //  the delay defined by the redis variable 'owntone' 'unmute_delay'
            //  $args may also contain 'delay' or delay-all = unmute all with a delay
            // used to temporarily mute the master volume when pausing or stopping mpd play or switching players
            //  when this is not done owntone will continue playing its buffer until it is empty
            //  a fixed delay between the mute and unmute action is currently used, defined by the redis variable 'owntone' 'unmute_delay'
            //      (maybe a smarter way of working is required based on the 'owntone' 'server_player' 'state' which changes from
            //          'play' to 'paused' when the buffer is empty)
            // this async version of the function starts the command file 'owntone_unmute.php' asynchronously which in turn
            //  calls wrk_owntone($redis, 'mute', $args);
            // check that owntone is running
            if (!$redis->hGet('owntone', 'active') || !wrk_systemd_unit($redis, 'is-active', 'owntone')) {
                // owntone is not running, do nothing
                break;
            }
            ...
            if (isset($args)) {
                $args = trim($args);
                if (substr($args, 0, 5) == 'delay') {
                    $args = substr($args, 5);
                    $delay = $redis->hGet('owntone', 'unmute_delay');
                }
            } else {
                $args = '';
            }
...
// when it is a dual configuration the the shairport-sync and shairport-sync-ap1 versions are switched when owntone starts and stops
//  when owntone is started the symlink shairport-sync.service must point to shairport-sync-ap1.service
//  when owntone is stopped the symlink shairport-sync.service must point to shairport-sync-ap2.service
{
    $shairportSyncBinary = trim(sysCmd('which shairport-sync')[0]);
    $shairportSyncAp1Binary = trim(sysCmd('which shairport-sync-ap1')[0]);
    $filename = '/etc/systemd/system/shairport-sync.service';
    clearstatcache(true, $filename);
    if ($shairportSyncBinary && $shairportSyncAp1Binary) {
        ...
        if ($cores == 1) {
            ...
            // multiprocessor - shairport-sync-ap1 or  shairport-sync will be started depending on the enabled status of owntone
            // 'dual' configuration using the shairport-sync-ap1.service or shairport-sync-ap2.service
            $redis->hSet('airplay', 'ss_conf', 'dual');
            if ($redis->hGet('owntone', 'enable')) {
                // owntone is enabled the symlink shairport-sync.service points to shairport-sync-ap1.service
                ...
                // owntone is disabled the symlink shairport-sync.service points to shairport-sync-ap2.service
                // check that the systemd unit file for shairport-sync-ap2 exists
                $filename1 = '/etc/systemd/system/shairport-sync-ap2.service';
                clearstatcache(true, $filename1);
                if (!file_exists($filename1)) {
...
//  $action = 'picture'
//      $args is an array containing type, code, url and optional filename:
...
//      For pictures we avoid reading the payload into php, as the file size is unknown and could be very large.
...
//  snapcast_server     uses loopback device 0 subdevice 1 input and device 1 subdevice 1 output
