                ...
                <?php if (isset($controls[$classification]) && count($controls[$classification])) :?>
                    <?php foreach ($controls[$classification] as $l) :?>
                        ...
                        <div style="width:max(55%,500px); min-height:70px;" class="boxed">
                            ...
                            <button id="<?=$l['id']?>ConnectButton" name="<?=$l['id']?>ConnectButton" type="button" <?php if ($l['selected']): ?>style="float:right;" <?php endif;?> title="Connect the output" class="btn btn-primary btn-lg<?php if (($l['selected'] && $l['autoconnect']) || (!$multidevice && (($l['type'] == 'ALSA') || ($l['type'] == 'PipeWire')))): ?> hide<?php endif;?>" value="1"><?php if (!$l['selected']): ?> Connect<?php else:?> Disconnect<?php endif;?></button>
