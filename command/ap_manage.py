...
def trigger_wifi_scan():
    ...
    if wifi_manager == "iwd":
        ...
        return trigger_wifi_scan_wpa()
    else:
        print("No WiFi manager running...")

def trigger_wifi_scan_iwd():
    """Trigger a WiFi scan using IWD via D-Bus."""
...
#        print(f"Found {len(station_paths)} station interfaces: {station_paths}")
        ...
        for path in device_paths:
            ...
            props_iface = dbus.Interface(device_obj, 'org.freedesktop.DBus.Properties')
            try:
                # In iwd, the device interface name is stored in the 'Name' property
                device_name = props_iface.Get('net.connman.iwd.Device', 'Name')
#                print(f"Device at {path} has name: {device_name}")
                if device_name == "wlan0":
                    wlan0_device_path = path
                    break
            except dbus.exceptions.DBusException as e:
...
#            print(f"Using first available station: {path}")
            station_obj = bus.get_object('net.connman.iwd', path)
            station_iface = dbus.Interface(station_obj, 'net.connman.iwd.Station')
            station_iface.Scan()
#            print(f"IWD scan triggered on first available station ({path})")
        ...
        print(f"[iwd] Unexpected error in trigger_wifi_scan_iwd: {e}")
        import traceback
        traceback.print_exc()
        return False

def trigger_wifi_scan_wpa():
    """Trigger a WiFi scan using wpa_supplicant."""
    print("[wpa] Triggering wifi scan via wpa_cli...")
    try:
        # Determine the best way to call wpa_cli
        wpa_cli_cmd = ["wpa_cli"]
        if os.path.exists("/run/wpa_supplicant/wlan0"):
        ...
        print(f"Error triggering WPA scan: {e}")
