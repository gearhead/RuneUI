...
if ($fileError) {
    if (isset($errorMessages[$fileError])) {
        ui_notifyError($redis, 'Restore', 'Error! '.$errorMessages[$fileError]);
    } else {
        ui_notifyError($redis, 'Restore', 'Error! Unknown error, error number: '.$fileError);
    }
    $isError = true;
} else if ($fileSize === 0) {
    ui_notifyError($redis, 'Restore', 'Error! Empty file.');
    $isError = true;
} else if (strpos($fileType, 'image')) {
    ui_notifyError($redis, 'Restore', 'Error! Invalid file contents.');
    $isError = true;
    ...
    ui_notifyError($redis, 'Restore', 'Error! File extension incorrect, expected: .tar.gz');
    $isError = true;
} else if (!move_uploaded_file($fileTmp, $fileDest)) {
    ui_notifyError($redis, 'Restore', 'Error! Invalid upload, cannot process file.');
    $isError = true;
