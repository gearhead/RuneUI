...
count=$( curl -X GET -s -f --connect-timeout 5 -m 10 --retry 1 "https://developer.jamendo.com/v3.0/radios/stream" | grep -c "WARNING: The stream link returned is not more working, and it could be never fixed" )
if [ $count -eq 0 ]; then
