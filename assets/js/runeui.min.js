...
function refreshTimer(startFrom,stopTo=null,state){var display=$('#countdown-display');if(typeof display.countdown!=="undefined"){display.countdown('destroy');display.countdown({since:((state!=='stop'||state!==undefined)?-(startFrom):0),compact:true,format:'MS'});if(state!=='play'){display.countdown('pause');}}
var displayss=$('#countdown-display-ss');if(typeof displayss.countdown!=="undefined"){displayss.countdown('destroy');displayss.countdown({since:((state!=='stop'||state!==undefined)?-(startFrom):0),compact:true,format:'MS'});if(state!=='play'){displayss.countdown('pause');}}
var displaysss=$('#countdown-display-sss');if(typeof displaysss.countdown!=="undefined"){displaysss.countdown('destroy');displaysss.countdown({since:((state!=='stop'||state!==undefined)?-(startFrom):0),compact:true,format:'MS'});if(state!=='play'){displaysss.countdown('pause');}}}
function refreshKnob(){window.clearInterval(GUI.currentKnob);if(typeof GUI.json.song_percent!=='undefined'){var initTime=parseInt(GUI.json.song_percent)*10;}
if(typeof GUI.json.time!=='undefined'){var delta=parseInt(GUI.json.time);var step=parseInt(1000/delta);}
var el=$('.countdown-amount').html();var time=$('#time');time.val(initTime,false).trigger('update');if(GUI.state==='play'){GUI.currentKnob=setInterval(function(){initTime=initTime+((GUI.visibility!=='visible')?step:1);time.val(initTime,false).trigger('update');},delta);}}
...
function setUIbuttons(activePlayer){if($('#section-index').length){document.getElementById('volume').readOnly=true;if((activePlayer==='Spotify')||(activePlayer==='Airplay')||(activePlayer==='SpotifyConnect')||(activePlayer==='Bluetooth')){$('#volume-knob').addClass('disabled');$('#volume-knob').addClass('nomixer');$('#volume-knob button').prop('disabled',true);$('#volume').addClass('disabled');$('#volume-knob').trigger('configure',{'readOnly':true,'fgColor':'#1A242F'}).css({'color':'#1A242F','pointer-events':'none'});$('#volume').trigger('configure',{'readOnly':true,'fgColor':'#1A242F'}).css({'color':'#1A242F','pointer-events':'none'});$('#volumedn').addClass('disabled');$('#volumemute').addClass('disabled');$('#volumeup').addClass('disabled');$('#repeat').addClass('hide');$('#random').addClass('hide');$('#single').addClass('hide');$('#previous').addClass('hide');$('#stop').addClass('disabled');$('#play').addClass('disabled');$('#next').addClass('disabled');}else{if(GUI.stream){$('#repeat').addClass('hide');$('#random').addClass('hide');$('#single').addClass('hide');}else{$('#repeat').removeClass('hide');$('#random').removeClass('hide');$('#single').removeClass('hide');}
if(GUI.consume==='1'){$('#previous').addClass('hide');}else{$('#previous').removeClass('hide');}
...
$('#stop').removeClass('disabled');$('#play').removeClass('disabled');$('#next').removeClass('disabled');}}else{if((activePlayer==='Spotify')||(activePlayer==='Airplay')||(activePlayer==='SpotifyConnect')||(activePlayer==='Bluetooth')){$('#stop').addClass('disabled');$('#play').addClass('disabled');$('#next').addClass('disabled');$('#previous').addClass('hide');}else{$('#stop').removeClass('disabled');$('#play').removeClass('disabled');$('#next').removeClass('disabled');if(GUI.consume==='1'){$('#previous').addClass('hide');}else{$('#previous').removeClass('hide');}}
GUI.activePlayer='';}}
...
if(GUI.stream){$('#total').html('<span>&infin;</span>');$('#total-ss').html('<span>&infin;</span>');$('#total-sss').html('<span>&infin;</span>');}else{$('#total').html('00:00');$('#total-ss').html('00:00');$('#total-sss').html('00:00');}
...
function updateGUI(){var volume=((typeof GUI.json.volume=='undefined')?0:GUI.json.volume);var radioname=((typeof GUI.json.radioname=='undefined')?'':GUI.json.radioname);var currentartist=((typeof GUI.json.currentartist=='undefined')?'':GUI.json.currentartist);var currentsong=((typeof GUI.json.currentsong=='undefined')?'':GUI.json.currentsong);var currentalbum=((typeof GUI.json.currentalbum=='undefined')?'':GUI.json.currentalbum);var currentalbumartist=((typeof GUI.json.currentalbumartist=='undefined')?'':GUI.json.currentalbumartist);var currentcomposer=((typeof GUI.json.currentcomposer=='undefined')?'':GUI.json.currentcomposer);var mainArtURL=((typeof GUI.json.mainArtURL=='undefined')?'':GUI.json.mainArtURL);var smallArtURL=((typeof GUI.json.smallArtURL=='undefined')?'':GUI.json.smallArtURL);var bigArtURL=((typeof GUI.json.bigArtURL=='undefined')?'':GUI.json.bigArtURL);var coverArtPreload=((typeof GUI.json.coverArtPreload=='undefined')?'':GUI.json.coverArtPreload);var song_lyrics=((typeof GUI.json.song_lyrics=='undefined')?'':GUI.json.song_lyrics);var artist_bio_summary=((typeof GUI.json.artist_bio_summary=='undefined')?'':GUI.json.artist_bio_summary);var artist_similar=((typeof GUI.json.artist_similar=='undefined')?'':GUI.json.artist_similar);var activePlayer=((typeof GUI.json.actPlayer=='undefined')?'':GUI.json.actPlayer);if(radioname!==null&&radioname!==undefined&&radioname!==''){var stream='radio';}else if(activePlayer!==undefined&&activePlayer==='Bluetooth'){var stream='bluetooth';}else{var stream='';}
GUI.stream=stream;if(typeof GUI.json.consume!=='undefined'){GUI.consume=GUI.json.consume;}
if((activePlayer!==GUI.activePlayer)||(volume!==GUI.volume)){GUI.activePlayer=activePlayer;setUIbuttons(activePlayer);}
if(GUI.stream){var time=100;}else if(typeof GUI.json.time!=='undefined'){var time=GUI.json.time;}
if(typeof GUI.json.elapsed==='undefined'){GUI.json.elapsed=0;var GUIcountdown=$('.countdown-amount').html();if(typeof GUIcountdown!=='undefined'){if(GUIcountdown.includes(':')){const GUIcountdownParts=GUIcountdown.split(':');if(GUIcountdownParts[0]===''){GUIcountdownParts[0]='0';}
if(GUIcountdownParts[1]===''){GUIcountdownParts[1]='0';}
GUI.json.elapsed=(parseInt(GUIcountdownParts[0])*60)+parseInt(GUIcountdownParts[1]);}}}
if((typeof GUI.json.song_percent==='undefined')&&(typeof GUI.json.elapsed!=='undefined')&&(typeof time!=='undefined')){if((GUI.json.elapsed!==0)&&(time!==0)){GUI.json.song_percent=parseInt((parseInt(GUI.json.elapsed)*100)/parseInt(time));}}
if(typeof GUI.json.elapsed!=='undefined'){refreshTimer(parseInt(GUI.json.elapsed),0,GUI.json.state);}
refreshState();if($('#section-index').length){if((mainArtURL!=='')&&(GUI.mainArtURL===mainArtURL)&&!$('#cover-art').css('background-image').includes(mainArtURL)){GUI.currentalbum_and_artist='';GUI.currentartist='';GUI.currentsong='';GUI.currentalbum=''
GUI.song_lyrics='';GUI.mainArtURL='';GUI.smallArtURL='';GUI.bigArtURL='';GUI.coverArtPreload='';GUI.artist_bio_summary='';GUI.artist_similar='';}
var currentalbum_and_artist=currentalbum+currentartist;if(GUI.currentalbum_and_artist!==currentalbum_and_artist){GUI.currentalbum_and_artist=currentalbum_and_artist;countdownRestart(GUI.json.elapsed);if($('#panel-dx').hasClass('active')){var current=parseInt(GUI.json.song);customScroll('pl',current);}}
...
function renderUI(text){toggleLoader('close');GUI.json=text[0];GUI.state=GUI.json.state;updateGUI();if($('#section-index').length){if(typeof GUI.json.time!=='undefined'){var time=GUI.json.time;}
if(typeof GUI.json.elapsed!=='undefined'){var elapsed=GUI.json.elapsed;}
if(typeof GUI.json.elapsed!=='undefined'){refreshTimer(parseInt(GUI.json.elapsed),0,GUI.json.state);}
refreshKnob();if(GUI.json.playlist!==GUI.playlist){getPlaylistCmd();GUI.playlist=GUI.json.playlist;}}
checkPlayerState()}
