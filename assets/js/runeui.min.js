...
function refreshTimer(startFrom,stopTo,state){var display=$('#countdown-display');if(typeof display.countdown!=="undefined"){display.countdown('destroy');display.countdown({since:((state!=='stop'||state!==undefined)?-(startFrom):0),compact:true,format:'MS'});if(state!=='play'){display.countdown('pause');}}
...
$('#repeat').removeClass('disabled');$('#random').removeClass('disabled');$('#single').removeClass('disabled');$('#previous').removeClass('disabled');$('#stop').removeClass('disabled');$('#play').removeClass('disabled');$('#next').removeClass('disabled');}}else{if((activePlayer==='Spotify')||(activePlayer==='Airplay')||(activePlayer==='SpotifyConnect')||(activePlayer==='Bluetooth')){$('#stop').addClass('disabled');$('#play').addClass('disabled');$('#next').addClass('disabled');}else{$('#stop').removeClass('disabled');$('#play').removeClass('disabled');$('#next').removeClass('disabled');}
GUI.activePlayer='';}}
...
if((GUI.stream==='radio')||(GUI.activePlayer==='Bluetooth')){$('#total').html('<span>&infin;</span>');$('#total-ss').html('<span>&infin;</span>');$('#total-sss').html('<span>&infin;</span>');}else{$('#total').html('00:00');$('#total-ss').html('00:00');$('#total-sss').html('00:00');}
...
if((typeof GUI.json.time!=='undefined')&&(typeof GUI.json.elapsed!=='undefined')){refreshTimer(parseInt(GUI.json.elapsed),parseInt(GUI.json.time),GUI.json.state);}
...
if((currentalbum==='')||(currentalbum===null)){var currentalbum='<span class="notag">[no album]</span>';}else if(GUI.currentalbum!==currentalbum){GUI.currentalbum=currentalbum;if(GUI.currentalbum.length>55){$('#currentalbum-ss')[0].style.fontSize="18px";$('#currentalbum-sss')[0].style.fontSize="28px";}else if(GUI.currentalbum.length>45){$('#currentalbum-ss')[0].style.fontSize="20px";$('#currentalbum-sss')[0].style.fontSize="30px";}else if(GUI.currentalbum.length>30){$('#currentalbum-ss')[0].style.fontSize="22px";$('#currentalbum-sss')[0].style.fontSize="32px";}else{$('#currentalbum-ss')[0].style.fontSize="24px";$('#currentalbum-sss')[0].style.fontSize="34px";}
...
console.log('mainArtURL = ',mainArtURL);console.log('GUI.mainArtURL = ',GUI.mainArtURL);console.log('UI = ',$('#cover-art').css('background-image'));if((mainArtURL!=='')&&(GUI.mainArtURL!==mainArtURL)){console.log('New mainArtURL = ','true');GUI.mainArtURL=mainArtURL;$('#cover-art').css('background-image','url('+mainArtURL+')');$('#cover-art-sss').css('background-image','url('+mainArtURL+')');}
...
if((typeof GUI.json.time!=='undefined')&&(typeof GUI.json.elapsed!=='undefined')){refreshTimer(parseInt(GUI.json.elapsed),parseInt(GUI.json.time),GUI.json.state);}
if((GUI.stream!='radio')&&(GUI.activePlayer!='Bluetooth')){refreshKnob();}else{$('#time').val(0,false).trigger('update');}
...
function onreleaseKnob(value){if(GUI.state!=='stop'&&GUI.state!==''){if((GUI.stream!='radio')&&(GUI.activePlayer!='Bluetooth')){window.clearInterval(GUI.currentKnob);var seekto=Math.floor((value*parseInt(GUI.json.time))/1000);sendCmd('seek '+GUI.json.song+' '+seekto);$('#time').val(value);$('#countdown-display').countdown('destroy');$('#countdown-display-ss').countdown('destroy');$('#countdown-display-sss').countdown('destroy');$('#countdown-display').countdown({since:-seekto,compact:true,format:'MS'});$('#countdown-display-ss').countdown({since:-seekto,compact:true,format:'MS'});$('#countdown-display-sss').countdown({since:-seekto,compact:true,format:'MS'});}else{$('#time').val(0).trigger('change');}}}
